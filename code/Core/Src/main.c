  ...
  *******************************/
  ...
  while (1)
  {
    ...
    if (HAL_GetTick() - last_debug_time >= 200) 
    {
      ...
      printf("ANGLE: %.2f | PID: %.2f | RPM: %.2f | STATE: %s | CURR q: %.2f | CURR d: %.2f\r\n", 
             angle, output, esc1_data.speed_rpm, GetStateName(esc1_data.status), esc1_data.current_Iq, esc1_data.current_Id);
    }
